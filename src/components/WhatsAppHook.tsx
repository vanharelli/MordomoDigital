...
const WhatsAppHook: React.FC = () => {
  ...
  const handleVipAccess = (e: React.MouseEvent) => {
    ...
    setTimeout(() => {
      ...
      const file = new File([vcard], "AlfaPlaza.vcf", { type: "text/x-vcard" }); 
      const url = URL.createObjectURL(file); 
      
      // Dispara a abertura direta no sistema mobile (Android/iOS)
  ...
  return (
    <div className="fixed bottom-6 right-6 z-[100] animate-bounce-subtle">
      <button
        ...
        disabled={isLoading}
        className="group relative flex items-center gap-4 bg-obsidian border border-gold p-4 rounded-2xl shadow-laser hover:scale-105 transition-all duration-300 max-w-[320px] text-left disabled:opacity-80"
      >
        {/* Glow Effect */}
